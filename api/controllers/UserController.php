...
// TAMBAHAN WAJIB: Pastikan ia jumpa fail database yang betul
...
class UserController {
    ...
    public function __construct() {
        // Guna fungsi getConnection(), BUKAN new Database()
        $this->conn = getConnection();
    }

    public function getAll() {
        $result = $this->conn->query("SELECT id, name, email, role, created_at FROM users ORDER BY id ASC");
        $users  = [];
    ...
    public function create() {
        ...
        $role     = $data['role'] ?? 'customer'; // Default kepada customer
    ...
    public function login() {
        ...
        // Menyokong password yang di-hash ATAU plain text (supaya akaun lama masih boleh login)
        if (password_verify($password, $user['password']) || $password === $user['password']) {
            unset($user['password']); // Jangan hantar password ke browser untuk keselamatan
